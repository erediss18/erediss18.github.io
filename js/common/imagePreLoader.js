...
var preLoad = function () {
    ...
    var onComplete = function () {
        ...
        var element = document.querySelector("#images-loading");
        var display = (element.currentStyle) ? element.currentStyle.display : getComputedStyle(element, null).display;

        if (nProcessed >= yourImages.length && (display != 'none')) {
            (document.querySelector("#images-loading")).style.display = 'none';
            (document.querySelector("#task-2-instructions")).style.display = 'block';
        }
    }
